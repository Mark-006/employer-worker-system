...
public class JobPanel extends JPanel implements Observer, Serializable, ActionListener{
	...
	public JobPanel() {
		...
		employerComboBox = new JComboBox<Employer>(EmployerDAO.getInstance().list().toArray(new Employer[0]));
        employerComboBox.setBounds(LX + employerLabel.getWidth() + LHS, employerLabel.getY(), TW, TH);
        employerComboBox.addItemListener(new ItemListener() {
        @Override
        public void itemStateChanged(ItemEvent e) {
        // Ensure descriptionTextArea is accessible and has components
        if (descriptionTextArea.getViewport().getComponentCount() > 0) {
            // Request focus on the first component inside the viewport of descriptionTextArea
...
//			} 
	...
	public void update() {
		...
		employerComboBox.setModel(new DefaultComboBoxModel<>(EmployerDAO.getInstance().list().toArray(new Employer[0])));
